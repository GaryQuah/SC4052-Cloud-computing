...
def rebuild_graph():
    """
    Nodes are entities/topics. Edges are triples between them.
    Multiple meetings can share the same entity nodes — this cross-meeting
    connectivity is what makes PageRank meaningful.

      "payment API" <--has_status-- "meeting 1"
      "payment API" <--depends_on-- "meeting 2"
    """
    G.clear()
...
Query: {query}"""}],
    ...
    # Step 2: Fuzzy-match entities to graph nodes
    nodes = list(G.nodes())
    seed_nodes = {
